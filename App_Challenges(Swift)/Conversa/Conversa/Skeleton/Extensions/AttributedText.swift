...
extension String {
    ...
    internal enum MobileFormat: String {
        case XXX_XXX_XXXX = "(XXX) XXX-XXXX"
    }
    ...
    func formatMobileNumber(with format: MobileFormat) -> String {
        let numbers = self.replacingOccurrences(of: "[^0-9]", with: "", options: .regularExpression)
        var result = ""
        var index = numbers.startIndex // numbers iterator
        let phonePattern = #"^\(?\d{3}\)?[ -]?\d{3}[ -]?\d{4}$"#

        let checkValidNumber = self.range(of: phonePattern, options: .regularExpression)

        let validPhoneNumber = (checkValidNumber != nil)
        if validPhoneNumber {
            return self
        }
        
        // iterate over the mask characters until the iterator of numbers ends
        for ch in format.rawValue where index < numbers.endIndex {
            if ch == "X" {
